...
extension CameraModel: AVCaptureMetadataOutputObjectsDelegate {
    nonisolated func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        Task { @MainActor in
            ...
            if let stringValue = firstString {
                ...
                self.recordDetectedCode(stringValue)
    ...
    private func recordDetectedCode(_ content: String) {
        guard !detectedCodes.contains(where: { $0.content == content }) else { return }
        detectedCodes.insert(DetectedCode(content: content, detectedAt: Date.now), at: 0)
    }
}
