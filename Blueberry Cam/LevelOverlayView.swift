...
struct LevelOverlayView: View {
    ...
    private func drawLevelMode(ctx: GraphicsContext, cx: CGFloat, cy: CGFloat, w: CGFloat) {
        ...
        ctx.drawLayer { layerCtx in
            ...
            layerCtx.translateBy(x: -cx, y: -cy)
            layerCtx.stroke(barPath,
                            with: .color(barColor.opacity(0.92)),
    ...
    private func drawFlatMode(ctx: GraphicsContext, cx: CGFloat, cy: CGFloat) {
        ...
        drawCrosshair(ctx: ctx, x: cx, y: cy,
                      color: .white.opacity(0.85), armLen: armLen, lineWidth: lw)
        ...
        drawCrosshair(ctx: ctx,
                      x: cx + dx,
                      y: cy + dy,
                      color: .yellow.opacity(model.isCrosshairAligned ? 1.0 : 0.9),
                      armLen: armLen,
                      lineWidth: lw)
    }
    ...
    private func drawCrosshair(ctx: GraphicsContext, x: CGFloat, y: CGFloat,
                                color: Color, armLen: CGFloat, lineWidth: CGFloat) {
        var h = Path()
